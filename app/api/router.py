...
def healthcheck_ia(forzar: bool = False):
    """¿Está viva la clave de IA? Sin ella, Runi no responde NADA.

    Existe porque una clave inválida se manifestaba como funciones sueltas que fallaban en vez de
    como lo que era: los 17 servicios de IA caídos a la vez. Nunca devuelve la clave ni parte de ella.
    El diagnóstico vive en `salud_ia_service` para que el panel del docente lo comparta sin gastar
    una llamada al proveedor por cada visita.
    """
    from app.services import salud_ia_service as sia
    return sia.estado(forzar=forzar)
